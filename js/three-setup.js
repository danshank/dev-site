...
class PortfolioScene {
    ...
    createRoomWireframe() {
        ...
        // Ceiling outline
        const ceilingPoints = points.map(p => new THREE.Vector3(p.x, height / 2, p.z));
        ...
        for (let i = 0; i < segments; i++) {
            ...
            const pillarGeometry = new THREE.BufferGeometry().setFromPoints(pillarPoints);
    ...
    createControls() {
        this.controls = new CameraControls(this.camera, this.cssRenderer.domElement);
    }
    ...
    getFocusedPanel() {
        ...
        this.panels.forEach(panel => {
            ...
            if (angle < closestAngle && angle < Math.PI / 3) { // Within 60 degrees
                closestAngle = angle;
                closestPanel = panel;
            }
        });
